...
## Pipeline completo: imagen inicial → vídeo → TouchDesigner (con chroma key)

Flujo de referencia cuando el punto de partida es una imagen inicial (foto real, no generada) que se quiere convertir en imagen/vídeo editado, con fondo verde chroma para poder integrarlo luego con alfa en TouchDesigner.

```
1. Imagen inicial (foto real)

2. Photoshop — corregir perspectiva e iluminación
   → No dejar esto para el paso de upscale: el upscale no corrige geometría
     ni exposición, solo añade textura/nitidez sobre lo que ya hay.

3. ComfyUI — upscale para recuperar detalle
   → Preferir upscalers fieles al original (SeedVR2, o modelo simple
     4x_NMKD-Siax_200k.pth) en vez de diffusion con denoise alto,
     para no alucinar contenido antes de la máscara.

4. Photoshop — máscara + fondo chroma verde

5. ComfyUI — generación sobre el fondo verde
   → Imagen: img2img (Flux 2 Klein)
   → Vídeo: Wan 2.2 i2v

6. ComfyUI — upscale + interpolación del vídeo a resolución final (p.ej. 1920×1080)
   → Workflow Video-Upscaler-Next-Diffusion (ver sección abajo)
   → IMPORTANTE: se hace ANTES del keying, no después — el output es
     h264 (pix_fmt yuv420p), que no soporta canal alfa. Además RIFE
     interpola mejor sobre el color sólido del chroma que sobre bordes
     ya recortados con alfa (genera artefactos).

7. After Effects — Keylight para quitar el fondo verde
   → Aquí es donde se genera el canal alfa, no antes.

8. Export QuickTime ProRes/high quality with alpha

9. Media Encoder — conversión a HAP Alpha

10. TouchDesigner
```

**Nota sobre el color del chroma:** si el upscaler del paso 3 altera ligeramente tono/saturación, revisar que el verde pintado en el paso 4 siga siendo un chroma limpio (alta saturación, sin mezcla de tonos) para que Keylight lo trague bien en el paso 7.
